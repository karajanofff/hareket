    ...
    <section className="relative overflow-hidden">
      ...
      <div className="mx-auto grid min-h-[680px] max-w-7xl items-center gap-10 px-4 py-16 lg:grid-cols-[1.02fr_.98fr]">
        <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.55 }}>
          <span className="inline-flex items-center gap-2 rounded border border-cyan-300/30 bg-cyan-300/10 px-3 py-2 text-sm text-cyan-100"><TrafficCone size={16} /> Premium automotive-learning platforma</span>
          <h1 className="mt-6 max-w-3xl text-4xl font-black leading-tight text-white sm:text-6xl">Kompyuter grafikası járdeminde jol háreketi qaǵıydaların interaktiv úyreniń</h1>
          <p className="mt-5 max-w-2xl text-lg leading-8 text-slate-300">Jol belgileri, video dársler, grafikalıq materiallar hám test moduli arqalı qáwipsiz háreket mádeniyatın qáliplestiriwshi platforma.</p>
          <div className="mt-8 flex flex-wrap gap-3">
            <Link className="btn-primary" href="/videos"><Play size={18} />Video dárslerdi kóriw</Link>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.65, delay: 0.1 }} className="relative min-h-[520px]">
          ...
          <div className="absolute bottom-8 right-8 z-10 glass w-64 p-4">
            <div className="aspect-video rounded bg-slate-950 ring-1 ring-cyan-300/30 grid place-items-center"><Play className="text-cyan-200" size={42} /></div>
            <p className="mt-3 text-sm font-semibold text-white">Video dárs preview</p>
