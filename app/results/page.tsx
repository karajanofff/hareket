...
export default async function ResultsPage() {
  ...
  return <section className="page space-y-8"><h1 className="text-4xl font-black text-white">Nátiyjeler</h1><div className="grid gap-4 md:grid-cols-4"><StatCard title="Testler sanı" value={results.length} icon={BarChart3} /><StatCard title="Ortasha nátiyje" value={`${avg}%`} icon={CheckCircle2} /><StatCard title="Eń jaqsı nátiyje" value={`${best}%`} icon={Award} /><StatCard title="Video progress" value={`${watched}/12`} icon={Video} /></div><div className="glass p-6"><h2 className="mb-4 text-2xl font-black text-white">Progress grafik</h2><ProgressChart data={chart.length ? chart : [{ name: "Start", value: 0 }]} type="line" /></div></section>;
}
