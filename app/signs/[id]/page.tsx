...
export default async function SignDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <section className="page grid gap-8 lg:grid-cols-[360px_1fr]">
      <aside className="glass grid h-fit place-items-center p-8">
        {isImage ? <img src={sign.svgType} alt={sign.name} className="max-h-64 max-w-64 object-contain" /> : <RoadSign type={sign.svgType} className="h-56 w-56" />}
        <span className="badge mt-5">{sign.category}</span>
      </aside>
